...
func setupTestManifestNetwork(t *testing.T, ctx context.Context) (host.Host, *dht.IpfsDHT, *pubsub.PubSub) {
    // Create a test host
    h, err := libp2p.New()
...
func TestManifestBasicOperations(t *testing.T) {
    ...
    host, dht, ps := setupTestManifestNetwork(t, ctx)
    defer host.Close()
    defer dht.Close()
...
func TestManifestReplication(t *testing.T) {
    ...
    // Create two nodes
    host1, dht1, ps1 := setupTestManifestNetwork(t, ctx)
    defer host1.Close()
    defer dht1.Close()

    host2, dht2, ps2 := setupTestManifestNetwork(t, ctx)
    defer host2.Close()
    defer dht2.Close()
