...
func TestChunkStoreNetworkFailures(t *testing.T) {
	...
	bigData := make([]byte, 100*1024*1024) // 100MB chunk
	rand.Read(bigData)
	store1.Store("bighash", bigData)

	// Channel to signal when download starts
	...
	// Wait for download to start and then close connection
	<-started
	time.Sleep(100 * time.Millisecond) // Give a moment for transfer to begin
	host1.Network().ClosePeer(host2.ID())

	// Wait for download to complete and check error
	err := <-completed
	require.Error(t, err, "should fail when connection is closed")
	assert.Contains(t, err.Error(), "connection closed during transfer")
	...
	require.NoError(t, err)
	time.Sleep(500 * time.Millisecond) // Give more time for connection to stabilize
