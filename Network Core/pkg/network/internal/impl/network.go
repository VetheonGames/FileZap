...
func (n *networkImpl) Connect(addrStr string) error {
...
func (n *networkImpl) Bootstrap(bootstrapPeers []string) error {
    ...
    for _, addrStr := range bootstrapPeers {
        ...
        addr, err := ma.NewMultiaddr(addrStr)
        if err != nil {
            return fmt.Errorf("invalid bootstrap address %q: %w", addrStr, err)
        }
        peerInfo, err := peer.AddrInfoFromP2pAddr(addr)
        if err != nil {
            return fmt.Errorf("invalid peer address %q: %w", addrStr, err)
        }
...
func (n *networkImpl) GetPeers() []string {
    peers := n.engine.GetPeers()
