...
type Registry struct {
	...
	filesByName map[string]*FileInfo // map[fileName]FileInfo
	dataDir     string
	mu          sync.RWMutex
...
func (r *Registry) GetPeersForChunk(chunkID string) []string {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Registry) GetFileByID(id string) (*FileInfo, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Registry) GetFileByName(name string) (*FileInfo, bool) {
	...
	file, exists := r.filesByName[name]
	return file, exists
}
